...
export default function CarrierDashboard() {
  ...
  const {
    simMode, waypoints, previousSegments, routeGeometry, loadSimulationSegments,
  ...
  const handleStartSim = async () => {
    ...
    const started = startSimulation(selected._id, selected.trackingNumber, cId);
    if (!started) {
  ...
  const handleStopSim = async () => {
    if (!selected) return;
    const cId = simOverride || profile?._id;
    stopSimulation(selected.trackingNumber, cId);
  };
